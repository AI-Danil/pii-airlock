...
def create_app(service: AirlockService | None = None) -> FastAPI:
    airlock = service or AirlockService()
    app = FastAPI(title="PII Airlock", version="0.1.0", docs_url="/api/docs")
    ...
    def complete_operation(operation_id: str) -> dict[str, object]:
        try:
            ...
            LOGGER.info(json.dumps({"event": "completion_blocked", "operation_id": operation_id, "reason": type(exc).__name__}))
            raise HTTPException(status_code=422, detail=str(exc)) from exc
        LOGGER.info(json.dumps({"event": "completion_finished", "operation_id": operation_id, "cloud_status": result["cloud_status"]}))
        return result
...
def _lm_studio_health() -> dict[str, object]:
    try:
        ...
        return {"reachable": True, "models": model_ids}
