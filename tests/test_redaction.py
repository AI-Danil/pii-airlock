...
from pii_airlock.models import DetectionError, Entity, EntityType, TokenMode, UnknownTokenError
...
def test_redacts_longest_values_and_reuses_stable_token() -> None:
    ...
    result = redact_fields(fields, entities, nonce="A1B2C3D4", token_mode=TokenMode.TYPED)
...
def test_manual_span_redaction_does_not_expand_to_an_unselected_duplicate() -> None:
    ...
    result = redact_spans(
        ...
        nonce="A1B2C3D4",
        token_mode=TokenMode.TYPED,
    )
    assert result.sanitized_fields["text"] == "__PII_A1B2C3D4_PERSON_0001__ called Elena"
    assert len(result.redactions) == 1


def test_opaque_mode_hides_types_and_does_not_link_repeated_values() -> None:
    text = "Elena called Elena"
    result = redact_fields(
        {"text": text},
        [Entity("Elena", EntityType.PERSON)],
    ...
    tokens = result.sanitized_fields["text"].split(" called ")
    assert len(set(tokens)) == 2
    assert all("PERSON" not in token for token in tokens)
