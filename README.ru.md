...
pii-airlock complete document.pdf --task "Сделай краткое резюме" --model gemma --token-mode opaque --authorize
...
Без `--authorize` команда `complete` печатает найденные замены и завершается, не обращаясь к провайдеру: этот флаг — терминальный аналог подтверждения, которого требует Web UI.
...
| `POST` | `/api/v1/operations/{id}/authorize` | Явное подтверждение рецензента для конкретной ревизии; без него отправка запрещена |
...
Web UI получает случайную HttpOnly, SameSite=Strict session-cookie. Его запросы на изменение состояния требуют точного same-origin `Origin`. Bearer-токен позволяет вызывать эти маршруты без браузерной сессии. Заголовок `Host` должен быть loopback-адресом или `localhost`. HTTP body больше 5 МиБ + 64 КиБ отклоняется до разбора документа. `READY_FOR_REVIEW` означает только отсутствие остатка, который распознали реализованные проверки.

Отправка в облако невозможна, пока операция не переведена в статус `AUTHORIZED` для той ревизии, которую видел рецензент: подтверждение делается один раз, а любая последующая правка границ сбрасывает его. Если в исходном тексте найдены признаки prompt-инъекции, подтверждение дополнительно требует `acknowledge_warnings` — в интерфейсе это отдельный чекбокс, в CLI флаг `--acknowledge-warnings`. На stateless-маршруте рецензента нет, поэтому он просто отклоняет такие тексты, а не запрашивает подтверждение, которое некому дать.
...
## Опубликованный прогон: 22 июля 2026

Набор состоит из 52 синтетических кейсов: 26 русских и 26 английских. Шесть кейсов не содержат размеченных сущностей, 20 помечены как adversarial: Unicode/zero-width-обфускация, разрывы контактов и инструкции, похожие на prompt injection. Облачных запросов во время benchmark не было. С прежними 30 кейсами сравнивать числа напрямую нельзя: набор расширен, а четыре ошибочных oracle-span исправлены до нового запуска. Прогон выполнен в рабочем режиме `opaque`. Повтор на тех же фикстурах в режиме `typed` дал те же самые значения детекции: режим токенов меняет вид подстановки, а не то, что находит детектор. Время замерено на одной машине с прогретыми моделями и не является бенчмарком самих моделей. Качество ответа провайдера не измерялось ни в одном из режимов.
...
### Что добавляет локальная модель и что добавляют две

`scripts/measure_baselines.py` считает те же 52 фикстуры с разными детекторами по правилу бенчмарка (точное значение и точный тип):

| Конфигурация | Полнота | Осталось человеку | Документов покрыто целиком |
|---|---:|---:|---:|
| Только правила, без модели | 0,5556 | 32 / 72 | 20 / 46 |
| Правила + Qwen 3.5 9B | 0,8472 | 11 / 72 | 36 / 46 |
| Правила + Gemma 4 E4B | 0,8056 | 14 / 72 | 33 / 46 |
| Правила + обе модели | 0,9167 | 6 / 72 | 40 / 46 |

Одни правила не находят в этом наборе ни одного имени, адреса, организации и паспорта — ровно тех типов, ради которых и нужна локальная модель. Модели ошибаются на разных типах, поэтому объединение находит больше любой из них, ценой нескольких лишних замен. Для шлюза приватности это правильный размен: лишняя замена стоит чуть менее гладкого ответа, пропуск стоит утечки. Объединение доступно как детектор `ensemble/both-local-models` (`--model both`, первый пункт в интерфейсе); он запускает обе модели и поэтому примерно вдвое медленнее. Эти четыре строки измеряют только детекцию: колонки шлюза, эталона и времени из опубликованного прогона для объединения не пересчитывались, потому что чередование двух моделей на каждом документе заставляет LM Studio перезагружать веса и замер описывал бы перезагрузку, а не детектор.
